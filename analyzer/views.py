...
def upload_file(request):
    try:
        ...
        if not uploaded_file_name and request.session.get('uploaded_csv'):
            uploaded_file_name = os.path.basename(request.session.get('uploaded_csv'))
...
def repetition_view(request):
    try:
        ...
        df_pairs = pd.DataFrame({'Message1': df_assistant['Message'][:-1].values, 'Message2': df_assistant['Message'][1:].values}) if not df_assistant.empty else pd.DataFrame()
        ...
        return render(request, 'analyzer/repetition.html', {'avg_score': 0, 'repetitive_pairs': [], 'repetitive_pairs_json': '[]', 'similar_responses': None, 'query': '', 'error': 'Failed to compute repetition'})
